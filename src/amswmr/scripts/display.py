...
class AmsMapCanvas(FigureCanvas):
      ...
      if event.button == 1:
        try:
          ...
          if self._lastStamp != trans:
            ps = (trans.transform.translation.x*1000.0, trans.transform.translation.y*1000.0)
        ...
        if best is not None:
          ...
          if self._startTag is not None:
            msg = String()
            msg.data = '{},{}'.format(self._startTag, best[0])
  ...
  def _handleGoal(self, msg):
    print 'Handle goal'
    event = AmsEvent(msg.pose.position.x*1000.0, msg.pose.position.y*1000.0)
