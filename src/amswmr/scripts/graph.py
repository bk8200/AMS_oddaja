...
class Edge(object):
  ...
  def draw(self, axes=plt, c='k', off=0.0, lw=3, alpha=1.0):
  ...
  def connectNodes(edges, nodes):
    ...
    for name, edge in edges.iteritems():
      ...
      for n, node in nodes.iteritems():
        if np.sum((edge.p[0:2,0]-node.p)**2) < 4:
          start = n
        if np.sum((edge.p[0:2,-1]-node.p)**2) < 4:
          end = n
...
class Node(object):
  ...
  def draw(self, axes=plt):
    axes.plot(self.p[0], self.p[1], 'ko')
    axes.text(self.p[0]-TEXT_OFFSET, self.p[1]-TEXT_OFFSET, '{}'.format(self.name), ha='right', va='top', fontsize=10)
...
class Graph(object):
  def __init__(self): #, force=False
    #rp = rospkg.RosPack()
    #path = os.path.join(rp.get_path('amswmr'), 'scripts', 'world.pkl')
  ...
  def createDot(self, labels=True, positions=True):
    ans = ''
    ans += 'digraph space {\n'
    ans += '  graph [resolution=18, splines=true, esep=0.5];\n'
    ans += '  node [fontsize=180, penwidth=20, style=filled, fillcolor="#EEEEEE", shape=circle];\n'
    ans += '  edge [fontsize=180, penwidth=10, arrowsize=8];\n'
    
    ans += '\n  # Nodes\n'
    for n, p in self.nodes.nodes.iteritems():
      ans += '  {}'.format(n)
      if positions:
        ans += ' [pos="{},{}!"]'.format(p[0]/10.0, p[1]/10.0)
      ans += ';\n'
    
    ans += '\n  # Connections\n'
    for n, c in self.edges.connections.iteritems():
      ans += '  {} -> {}'.format(*c)
      if labels:
        ans += ' [label="{}"]'.format(n)
      ans += ';\n'  
  ...
  def drawAxes(self):
    ...
    self.axes.set_axis_bgcolor((0.9, 0.1, 0.9))
  ...
  def computeTagMap(self):
    self.tagMap = {}
    for k, link in self.links.iteritems():
  ...
  def draw(self, include='', start=None, goal=None, path=[], nodes=False, tags=True, labels=True):
    self.axes.cla()
    ...
    if not path:
      self.drawLinks(include=include)
    else:
      self.drawPath(path)
    Edge.drawEdges(self.edges, axes=self.axes)
    if nodes:
  ...
  def createFigure(self, fig=plt.figure):
    self.fig = fig(figsize=[x/25.4/5 for x in world.BOX[2:4]], facecolor=(0.9, 0.9, 0.9))
  ...
  def drawLink(self, v):
    if v[0] == 'X':
      ...
      c = 'k'
    edge = self.detEdges[v[2]]
    edge.draw(axes=self.axes, c=c, lw=10, alpha=0.3)

  def drawLinks(self, include='LXR'):
    for k, vv in self.links.iteritems():
      for v in vv:
        if v[0] in include:
          self.drawLink(v)
...
if __name__ == '__main__':
  ...
  print('The point {} on the edge \'{}\' is the closest to the point {}; distance between the points is {} mm. The point on the edge is {} mm away from the edge start point.'.format(opt[2], opt[0], point, opt[3], opt[1]))
  ...
  for name in pathEdges: # Print the length of each path edge
    print('Length of the edge \'{}\' is {} mm.'.format(name, graph.detEdges[name].length))

  plt.close('all')

  rp = rospkg.RosPack()
  path = os.path.join(rp.get_path('amswmr'), 'doc', 'img')
  ...
  with open(os.path.join(rp.get_path('amswmr'), 'scripts', 'graph_gen.py'), 'w') as f:
    f.write('#!/usr/bin/python\n# -*- coding: utf-8 -*-\n\n')
    f.write('tagMap = {\n')
    for i in graph.tagMap:
      f.write('  {}: {},\n'.format(i, graph.tagMap[i]))
    f.write('}\n')
    f.write('\n')
    f.write('tagPoses = {\n')
    for i, v in graph.tagPoses.iteritems():
      f.write('  {}: {},\n'.format(i, [float(x) for x in v]))
    f.write('}\n')
    f.write('\n')
    f.write('tagDets = {\n')
    for i, v in graph.detTags.iteritems():
      f.write('  {}: {},\n'.format(i, [float(x) for x in v.p]))
    f.write('}\n')
